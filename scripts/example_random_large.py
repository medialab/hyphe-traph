...
print(
    'Custom creation rules: "Hodor" is path-2 platform and "Lorem Ipsum" is path-1 platform'
)
...
print(
    "\t...%s page links added in %s ms"
    % (format(links_count, ",.0f"), format(1000 * add_links_duration, ",.0f"))
)

print("\n:: Stats")
print(
    " - %s webentities (from Store)"
    % (format(len(webentity_store.data["webentities"]), ",.0f"))
)
...
print(
    "\t...%s webentity links obtained in %s ms"
    % (
        format(webentity_links_count, ",.0f"),
        format(1000 * get_network_duration, ",.0f"),
    )
)
