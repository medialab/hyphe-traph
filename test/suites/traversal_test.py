...
class TestTraversal(TraphTestCase):
    ...
    def test_dfs_iter(self):
        with self.open_traph(
            ...
            partial_dfs = list(
                node.stem()
                for node, _ in trie.dfs_iter(
                    starting_node, b"s:http|h:com|h:world|p:europe|"
                )
            )

            self.assertEqual(
                partial_dfs, [b"p:europe|", b"p:spain|", b"p:france|", b"p:romania|"]
            )
    ...
    def test_webentity_inorder_iter(self):
        with self.open_traph(
            ...
            self.assertEqual(
                [
                    lru
                    for node, lru, _ in trie.webentity_inorder_iter(prefix_node, prefix)
                ],
                webentity_inorder,
            )
        ...
        with self.open_traph(
            ...
            for i, (pagination_lru, pagination_path) in enumerate(webentity_inorder):
                ...
                self.assertEqual(
                    [
                        lru
                        for node, lru, _ in trie.webentity_inorder_iter(
                            prefix_node, prefix, pagination_path
                        )
                    ],
        ...
        with self.open_traph(
            ...
            self.assertEqual(
                traph.paginate_webentity_pages(None, prefixes),
                {
                    ...
                    "count_crawled": 0,
                    "pages": [
                        {"lru": lru, "crawled": False} for lru, _ in webentity_inorder
                    ],
                },
            )
            ...
            self.assertEqual(
                traph.paginate_webentity_pages(
                    None, prefixes, page_count=len(webentity_inorder)
                ),
                {
                    "done": True,
                    "count": len(webentity_inorder),
            ...
            self.assertEqual(
                traph.paginate_webentity_pages(
                    ...
                    prefixes,
                    page_count=2,
                    pagination_token=build_pagination_token(0, ops_to_base4("CLR")),
                ),
                {
                    "done": False,
                    "count": 2,
                    ...
                    "pages": [
                        ...
                        {
                            "lru": b"s:http|h:com|h:world|p:europe|p:france|",
                            "crawled": False,
                        },
                    ],
            ...
            self.assertEqual(
                traph.paginate_webentity_pages(
                    None,
                    prefixes + [b"s:https|h:com|h:world|"],
                    page_count=20,
                    pagination_token=build_pagination_token(0, ops_to_base4("CLR")),
                ),
                {
            ...
            while True:
                result = traph.paginate_webentity_pages(
                    None, prefixes, page_count=3, pagination_token=token
                )
                calls += 1
            ...
            self.assertEqual(
                traph.paginate_webentity_pages(None, prefixes, crawled_only=True),
                {
                    ...
                    "count": 1,
                    "count_crawled": 1,
                    "pages": [{"lru": b"s:http|h:com|h:world|p:aaa|", "crawled": True}],
                },
            )
